...
First, install the 8528 kernel module to get the fan speed information. In Linux, you can install the ~lm-sensors~ package to get the CPU and HDD temperature. You can run ~sensors~ command to see the temperature. However, the fan speed information is not shown. As the QNAP uses a ITE8528 embedded controller to control the fan speed, the ~lm-sensors~ package does not support it. [[https://github.com/0xgiddi/qnap8528][qnap8528]] provides a kernel module to support this controller. Gzxiexl created a [[https://github.com/gzxiexl/qnap8528][a more conventient script]] to build, install and automatically load the kernel module. This script does not support PVE 9 currently, but I've created a [[https://github.com/gzxiexl/qnap8528/pull/1][PR]]. Before it's merged, you need to install the kernel headers with ~apt install proxmox-headers-$(uname -r)~ and edit the ~Dockerfile~ to use ~debian:13~ as base image. Then you can run the ~sudo build.sh~.
