...
***** Add Custom Features in the Text
...
***** Search Original Log from ElasticSearch
...
If there is a ~pythohn._pth~ or ~python314._pth~ in the same directory as Python executable, the isolated mode is enabled and ~site~ module is not imported. Isolated mode is the same as ~python3 -I~: the environment and registry variable are ignored, the ~site~ module is not imported and the [[https://docs.python.org/3/using/cmdline.html#cmdoption-P][unsafe path]] and [[https://docs.python.org/3/library/site.html#site.USER_SITE][USER_SITE]] are not added to ~sys.path~. After adding this file, only the path in the ~_pth~ file is added into ~sys.path~. This is a special use case for embedding Python. Here is the ~python314._pth~ file in embed Python 3.14:
...
If you use the native style or pkg_resource style but there is a normal ~__init__.py~ in ~sys.path~. Although the namespace package has been imported, Python still only returns the normal package and ignored the namespace package. That's why in the [[https://github.com/Azure/azure-cli/issues/31843][#31843 issue]], the ~azure.cli~ is not found.
...
*** DONE Lazy Import in Python 3.15
CLOSED: [2026-06-26 Fri 21:49]
   :PROPERTIES:
   :EXPORT_FILE_NAME: lazy-import-in-python-3-dot-15
   :END:
...
**** How to enable lazy import globally
...
As you know, some modules may have side effects when imported. If you want to key them import eagerly, you can use the ~sys.set_lazy_imports_filter()~ [[https://docs.python.org/3.15/library/sys.html#sys.set_lazy_imports_filter][import filter]].
...
**** Ref
- [[https://peps.python.org/pep-0810/][PEP 810 – Explicit lazy imports]]
- [[https://docs.python.org/3.15/whatsnew/3.15.html#pep-810-explicit-lazy-imports][What's New In Python 3.15]]
- [[https://docs.python.org/3.15/reference/datamodel.html#module.__lazy_modules__][module.__lazy_modules__]]
...
*** DONE Kindle Paperwhite 5 Review                                :Kindle:
CLOSED: [2026-01-09 Fri 21:49]
   :PROPERTIES:
   :EXPORT_FILE_NAME: kindle-paperwhite-5-review
   :END:
...
**** Kindle Jailbreak
If you want to read PDF on Kindle, KOReader is a must-have app. You can follow the [[https://kindlemodding.org/][Kindle Modding]] guide to jailbreak your Kindle and install KOReader.

There are some plugins I recommend:
1. [[https://github.com/guo-yong-zhi/kindle-filebrowser][Kindle FileBrowser]]: You can transfer books through wifi without using USB cable.
2. [[https://github.com/yparitcher/KUAL_Booklet/][KOL]]: Open KOReader from main UI.
3. [[https://www.mobileread.com/forums//showpost.php?p=4539994&postcount=33][Cover Setter]]: Set a better cover for KUAL/KOL/HOTFIX

For KOReader, here are the plugins I recommend:
1. [[https://github.com/doctorhetfield-cmd/simpleui.koplugin][SimpleUI]]: Improve home screen UI.
2. [[https://github.com/kristianpennacchia/zzz-readermenuredesign.koplugin][ReaderMenuRedesign]]: Improve the Lookup menu and highligh menu.

**** Kindle software
Besides the must have Calibre, there are two tools I recommend for Kindle users:
1. [[https://github.com/ystyle/kaf-cli][kaf-cli]]: A CLI tool to convert txt to epub, with a orly style cover.
2. [[https://github.com/ciromattia/kcc][Kindle Comic Converter]]: A tool to convert manga to mobi, which can be read on Kindle with full screen.

**** Ref:
1. [[https://kindlemodding.org/][Kindle Modding Wiki]]
2. [[https://bookfere.com/category/skills/jailbreak][书伴 越狱专题]]

*** DONE 在线起诉指南（人民法院在线服务）                           :court:
CLOSED: [2026-05-23 Sat 17:18]
   :PROPERTIES:
   :EXPORT_FILE_NAME: 在线起诉指南-人民法院在线服务
   :END:
...
*** DONE Control QNAP NAS Fan Speed in PVE                            :NAS:
CLOSED: [2026-07-07 Tue 21:16]
   :PROPERTIES:
   :EXPORT_FILE_NAME: control-qnap-nas-fan-speed-in-pve
   :END:
...
**** Install the 8528 Kernel Module
...
***** Option 1: Install with DKMS

[[https://github.com/0xgiddi/qnap8528][qnap8528]] provides a kernel module to support this controller. It contains the instruction to install with DKMS and [[https://github.com/0xgiddi/qnap8528#autoload-module-on-startup-with-systemd][autoload module on startup with systemd]]. Don't forget to install the pre-requisite packages: ~apt install build-essential pve-headers dkms~. You may need to install ~proxmox-headers-$(uname -r)~ if the kernel is not the latest version.

This methods automatically rebuild the kernel module when the kernel is updated.


***** Option 2: Build module in Docker
Gzxiexl created [[https://github.com/gzxiexl/qnap8528][a more conventient script]] to build, install and automatically load the kernel module. This script does not support PVE 9 currently, but I've created a [[https://github.com/gzxiexl/qnap8528/pull/1][PR]]. Before it's merged, you need to install the kernel headers with ~apt install proxmox-headers-$(uname -r)~ and edit the ~Dockerfile~ to use ~debian:13~ as base image. Then you can run the ~sudo build.sh~.
