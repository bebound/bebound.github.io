...
=socket.receive.buffer.bytes=(default value is 64KB) and =send.buffer.bytes=(default value is 128KB) is the =SO_RCVBUFF= and =SO_SNDBUFF= buffer for socket connections respectively. I recommend to set it to a bigger value or =-1= to use the OS default value.
