...
*** TODO Dynamic Allocate Executors when Executing Jobs in Spark
   :PROPERTIES:
   :EXPORT_FILE_NAME: dynamic-allocate-executors-when-executing-jobs-in-spark
   :END:
I wrote a Spark program to process logs. The number of logs always changes as time goes by. To ensure logs can be processed instantly, the number of executors is calculated by the maximum of logs per minutes. The CPU usage is low in executors.
